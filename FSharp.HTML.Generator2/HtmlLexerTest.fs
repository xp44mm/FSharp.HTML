...
type HtmlLexerTest(output: ITestOutputHelper) as this =
    let name =
        let x = this.GetType().Name
        x.Substring(0, x.Length - 4)

    let srcPath = Path.Combine(__SOURCE_DIRECTORY__, "html.fslex")
    ...
    member _.``验证``() =
        // actual
        let anal = HtmlLexer.anal
        let actions =
            HtmlLexer.actions (fun () -> "") (fun () -> "") (fun _ -> HtmlToken.EOF)
