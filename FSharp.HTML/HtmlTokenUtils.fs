...
//    | TAGSTART(name, attrs) when HtmlSchema.voidElements.Contains name ->
//        TAGSELFCLOSING(name, attrs)
//        |> fun value -> Some { token with value = value }
//    | TAGEND name when HtmlSchema.voidElements.Contains name -> None
//    | _ -> Some token
...
//        let pt2 =
//            {
//                index = offset + gps.[2].Index
//                length = gps.[2].Length
//                value = TAGEND tagName
//            }
//        yield pt2
...
//                let postok =
//                    { index = i; length = m.Length; value = TAGEND "style" }
//                yield postok
...
//            | Rgx "^[^/\"<]+" m ->
//                let postok =
//                    { index = i; length = m.Length; value = TEXT m.Value }
//                yield postok
...
//                let postok =
//                    { index = i; length = m.Length; value = TAGEND "script" }
//                yield postok
//                yield! restloop postok.adjacent rest.[postok.length ..]
...
//            | _ -> yield! loop postok.adjacent rest.[postok.length ..]
//        }
