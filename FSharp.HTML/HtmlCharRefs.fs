module FSharp.HTML.HtmlCharRefs
...
let substituteNamedCharacterReference(orig:string) =
    refs
    |> Map.tryFind orig
    |> Option.defaultValue orig

let substituteDecimalNumericCharacterReference(orig:string) =
    let num = orig.[2..orig.Length-2] // &#(<?num>\d+);
    let num = Int32.Parse(num, NumberStyles.None, CultureInfo.InvariantCulture)
    Char.ConvertFromUtf32 num

let substituteHexadecimalNumericCharacterReference(orig:string) =
    let num = orig.[3..orig.Length-2] // &#[xX](<?num>[\da-fA-F]+);
...
let rec tokenizeRawText (inp:string) =
    seq {
        ...
        | _ when inp.[0] = '&' -> 
            yield inp.[0..0]
            yield! tokenizeRawText inp.[1..]
        | _ -> failwithf "rest:%s" inp
    }

let unescapseRawText (rawText:string) =
    rawText
    |> tokenizeRawText
    |> String.concat ""

let skipTags =
    TagNames.voidElements + TagNames.rawTextElements

let rec unescapseNode (node:HtmlNode) =
    match node with
    | HtmlElement(tag, attrs, children) ->
        let attrs =
            attrs
            |> List.map(fun(n,v) -> n,unescapseRawText v)
        let children =
            if skipTags.Contains tag then
                children
            else
                children
                |> List.map(unescapseNode)
        HtmlElement(tag, attrs, children)
    | HtmlText v -> HtmlText(unescapseRawText v)
    | HtmlCData _ -> node
    | HtmlComment _ -> node
...
let escapeNormalText(text:string) =
