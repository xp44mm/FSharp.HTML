...
module TagLeftToken =
    ...
    let tokenize (offset: int) (inp: string) = // (restloop)
        let rec loop (i: int) (rest: string) =
            seq {
                match rest with
                | Rgx @"^<([-:\.\w]+)" x ->
                    ...
                    yield
                        {
                            index = offset
                            length = x.Length
                            value = LANGLE tagName
                        }
                    yield! loop (i + x.Length) (x.Result("$'"))

                | On tryWS x -> yield! loop (i + x.Length) (x.Result("$'"))
                    ...
                    yield
                        {
                            index = i
                            length = x.Length
                            value = ATTR_VALUE x.Groups.[1].Value
                        }
                    yield! loop (i + x.Length) (x.Result("$'"))
                    ...
                    yield
                        {
                            index = i
                            length = x.Length
                            value = ATTR_VALUE x.Groups.[2].Value
                        }
                    yield! loop (i + x.Length) (x.Result("$'"))

                | StartsWith "/>" x ->
                    let postok =
                        { index = i; length = x.Length; value = DIV_RANGLE }
                    //pickRestData postok.nextIndex rest.[postok.length..]
                    yield postok

                | StartsWith ">" x ->
                    let postok =
                        { index = i; length = x.Length; value = RANGLE }
                    //pickRestData postok.nextIndex rest.[postok.length..]
                    yield postok
