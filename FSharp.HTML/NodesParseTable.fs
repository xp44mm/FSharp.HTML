...
let rules:(string list*(obj list->obj))list = [
    ["children";"nodes"],fun(ss:obj list)->
        let s0 = unbox<HtmlNode list> ss.[0]
        let result:HtmlNode list =
            List.rev s0
        box result
    ["children"],fun(ss:obj list)->
        let result:HtmlNode list =
            []
        box result
    ["nodes";"nodes";"node"],fun(ss:obj list)->
        let s0 = unbox<HtmlNode list> ss.[0]
        let s1 = unbox<HtmlNode> ss.[1]
        let result:HtmlNode list =
            s1::s0
        box result
    ["nodes";"node"],fun(ss:obj list)->
    ...
    ["element";"TAGSELFCLOSING"],fun(ss:obj list)->
        ...
        box result
    ["element";"TAGSTART";"children";"TAGEND"],fun(ss:obj list)->
        let s0 = unbox<Position<HtmlToken>> ss.[0]
