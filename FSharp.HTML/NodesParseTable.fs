...
let rules = [|["children";"nodes"],"List.rev s0";["children"],"[]";["nodes";"nodes";"node"],"s1::s0";["nodes";"node"],"[s0]";["node";"TEXT"],"htmlText s0";["node";"COMMENT"],"htmlComment s0";["node";"CDATA"],"htmlCData s0";["node";"element"],"s0";["element";"TAGSELFCLOSING"],"let name,attrs = getNameAttributes s0\r\nHtmlElement(name, attrs,[])";["element";"TAGSTART";"children";"TAGEND"],"let name,attrs = getNameAttributes s0\r\nif name = getName s2 then\r\n    HtmlElement(name, attrs, s1)\r\nelse\r\n    failwith $\"unmatched tag:<{name}></{s2}>\""|]
...
let declarations = [|"TAGSELFCLOSING","Position<HtmlToken>";"TAGSTART","Position<HtmlToken>";"TAGEND","Position<HtmlToken>";"CDATA","Position<HtmlToken>";"COMMENT","Position<HtmlToken>";"TEXT","Position<HtmlToken>";"node","HtmlNode";"element","HtmlNode";"children","HtmlNode list";"nodes","HtmlNode list"|]
open System
open FslexFsyacc.Runtime
open FSharp.HTML
open FSharp.HTML.HtmlTokenUtils
