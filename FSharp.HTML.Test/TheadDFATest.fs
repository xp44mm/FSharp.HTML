...
type TheadDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"thead.fslex") // **input**
    ...
    member _.``4 = valid DFA``() =
        ...
        Should.equal y.nextStates TheadDFA.nextStates
        Should.equal y.header     TheadDFA.header
        Should.equal y.rules      TheadDFA.rules
