...
type OptionDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"option.fslex") // **input**
    ...
    member _.``4 = valid DFA``() =
        ...
        Should.equal y.nextStates OptionDFA.nextStates
        Should.equal y.header     OptionDFA.header
        Should.equal y.rules      OptionDFA.rules
