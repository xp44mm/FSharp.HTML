...
type RubyDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"ruby.fslex") // **input**
    let text = File.ReadAllText(filePath)
            
    [<Fact(Skip="once and for all!")>] // 
    member _.``1 - generate DFA``() =

        let name = "RubyDFA" // **input**
    ...
    member _.``2 - valid DFA``() =
        ...
        Should.equal y.nextStates RubyDFA.nextStates
        Should.equal y.header     RubyDFA.header
        Should.equal y.rules      RubyDFA.rules
