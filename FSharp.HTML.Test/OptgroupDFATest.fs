...
type OptgroupDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"optgroup.fslex") // **input**
    ...
    member _.``4 = valid DFA``() =
        ...
        Should.equal y.nextStates OptgroupDFA.nextStates
        Should.equal y.header     OptgroupDFA.header
        Should.equal y.rules      OptgroupDFA.rules
