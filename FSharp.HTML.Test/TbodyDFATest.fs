...
type TbodyDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"tbody.fslex") // **input**
    let text = File.ReadAllText(filePath)
            
    [<Fact(Skip="once and for all!")>] // 
    member _.``1 - generate DFA``() =
        let name = "TbodyDFA" // **input**
    ...
    member _.``2 - valid DFA``() =
        ...
        Should.equal y.nextStates TbodyDFA.nextStates
        Should.equal y.header     TbodyDFA.header
        Should.equal y.rules      TbodyDFA.rules
