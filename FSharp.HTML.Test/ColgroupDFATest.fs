...
type ColgroupDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"colgroup.fslex") // **input**
    ...
    member _.``4 = valid DFA``() =
        ...
        Should.equal y.nextStates ColgroupDFA.nextStates
        Should.equal y.header     ColgroupDFA.header
        Should.equal y.rules      ColgroupDFA.rules
