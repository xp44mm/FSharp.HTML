...
type DlDFATest(output:ITestOutputHelper) =
    let show res =
        res
        |> Literal.stringify
        |> output.WriteLine
    ...
    let filePath = Path.Combine(sourcePath, @"dl.fslex") // **input**
    ...
    member _.``2 - valid DFA``() =
        ...
        Should.equal y.nextStates DlDFA.nextStates
        Should.equal y.header     DlDFA.header
        Should.equal y.rules      DlDFA.rules
